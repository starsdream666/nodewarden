...
function toCredentialIdBytes(value: unknown): Uint8Array {
...
export async function createPasskeyCredential(publicKey: Record<string, any>): Promise<any> {
  const options: PublicKeyCredentialCreationOptions = {
    ...
    challenge: toCredentialIdBytes(publicKey.challenge),
    user: {
      ...publicKey.user,
      id: toCredentialIdBytes(publicKey?.user?.id),
    },
    excludeCredentials: Array.isArray(publicKey.excludeCredentials)
      ? publicKey.excludeCredentials.map((item: any) => ({ ...item, id: toCredentialIdBytes(item.id) }))
      : [],
  };
...
export async function requestPasskeyAssertion(publicKey: Record<string, any>): Promise<any> {
  const allowCredentials = Array.isArray(publicKey.allowCredentials) && publicKey.allowCredentials.length
    ? publicKey.allowCredentials.map((item: any) => ({ ...item, id: toCredentialIdBytes(item.id) }))
    : undefined;
